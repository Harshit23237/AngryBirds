...
//        circleShape.setRadius(bird.getWidth()/2);
    ...
    private void applyMagnetism(Bird bird, Body birdBody) {
        ...
        float left = birdPosition.x - radius;
        float right = birdPosition.x + radius;
        float bottom = birdPosition.y - radius;
        float top = birdPosition.y + radius;

        world.QueryAABB(fixture -> {
            Body affectedBody = fixture.getBody();

            if (affectedBody.getType() == BodyDef.BodyType.StaticBody || affectedBody == birdBody) {
                return true;
            }

            Vector2 force = birdPosition.cpy().sub(affectedBody.getPosition()).nor().scl(100000f);
    ...
    public void render(float delta) {
        ...
        if (birdBody != null) {
            ...
            if(currentBird.getTexturePath().equalsIgnoreCase("shake_bird.png")) {
                applyMagnetism(currentBird, birdBody);
            }
